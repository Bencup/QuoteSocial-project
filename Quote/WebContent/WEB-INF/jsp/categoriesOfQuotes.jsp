...
<div class="col-md-3"></div>
  ...
  <!-- *************************************content in body this here ************************************************************ -->	
			...
			<a href="javascript:void(0)" data-toggle="collapse" data-target="#demo" class="fontcolor1" > 
 			...
 			<span class="boardFont1">Update your new quotes</span>
 			</a>
 			<%-- <c:forEach var="boardDto" items="${boardDtoList}" >			
 				${boardDto.user.username }	
  			...
  			<div id="demo" class="collapse">
    			<form class="form-horizontal" action="commentAns.html" method="post">
					...
					<div class="row">
						<div class="col-md-6">
						<div class="panel panel-default boardColorPanel">
							<div class="panel-body bTextarea1">
								<textarea class="form-control bTextarea2" rows="5" id="detailPost" placeholder="Update You Quote feel like.."
								 name="detailPost" required="required"></textarea>
							</div>
						</div>
						</div>
						<div class="col-md-6">
						<span class="bText1">Categories Quotes</span>
						<select class="form-control bSelectWidth" name="categoriesId">
							<c:forEach var="categorie" items="${categoriesList}" >
							<option value="${categorie.id}" >${categorie.categoriesName}</option>
							</c:forEach>	
   					  </select>
   					  	<p></p>
   					  	<span class="bText1">Author Name</span>
   					  	<input  type="text" class="form-control bSelectWidth" name="creditAuthorName" />
   					 	</div>
   					 	
   					 	
   					 </div>
   					  	<div>
							<button class="btn btn-info bcb" type="submit" >Post</button>
						</div>
					</form>
			...
			<div class="panel-default">
				...
				<c:forEach var="boardDto" items="${boardDtoList}">
					<c:if test="${boardDto.categories.id == categoriesOfquote.id}">
